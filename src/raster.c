#include "raster.h"
#include "aabb.h"
#include "transform.h"

void rt_ctx_clear(rt_ctx_t *rt_ctx)
{
    memset(rt_ctx->fbuffer, 0, sizeof(rt_ctx->fbuffer));

    for (int i = 0; i < SCREEN_WIDTH*SCREEN_HEIGHT; i++) {
        rt_ctx->zbuffer[i] = FLT_MAX;
    }
}

result_t rt_ctx_init(rt_ctx_t **out_rt_ctx)
{
    rt_ctx_t *rt_ctx = 
        (rt_ctx_t*)malloc(sizeof(*rt_ctx));
    if (!rt_ctx) {
        ERROR("rt ctx init allocation failed");
        return RESULT_ERROR_ALLOC;
    }
    rt_ctx_clear(rt_ctx);
    *out_rt_ctx = rt_ctx;

    return RESULT_OK;
}

void rt_ctx_destroy(rt_ctx_t *rt_ctx)
{
    free(rt_ctx);
}

void rt_put_pixel(rt_ctx_t *rt_ctx, 
                      i32 x, i32 y, f32 z, u32 c)
{
    if (!IN_BOUNDS(x, y)) {
        DEBUG("pixel out of bounds: {%d, %d}", x, y);
        return;
    }
    u32 idx = (y * SCREEN_WIDTH) + x;
    if (!rt_ctx->zbuffer[idx] || rt_ctx->zbuffer[idx]> z) {
        rt_ctx->zbuffer[idx] = z;
        rt_ctx->fbuffer[idx] = c;
    }
}

void rt_put_pixel_vec(rt_ctx_t *rt_ctx, v3 v, u32 c)
{
    i32 x = (i32)v.x;
    i32 y = (i32)v.y;

    rt_put_pixel(rt_ctx, x, y, v.z, c);
}

//this function works as follows: 
//youve got two points, p0, p1,
//depending on the slope of the
//line between these points, we 
//do one of these two things:
//(1) for every x check if we change y by 1
//              OR
//(2) for every y check if we change x by 1

//1 is the default, but we check `is_steep`
//in which case we swap the x and y to
//enable us to do 2.
 
//The second SWAP makes sure that we 
//are always drawing left->right

//`diff` keeps track of how much we
//have deviated from our original
//axis, which lets us know if we 
//should change the second axis by 1.

//The 'first' axis is the one being
//controlled in the for loop, and
//the `second` axis is the one 
//that changes by one when the 
//diff deviation surpasses the
//delta of the second axis, then
//we subtract 2*(delta(2)-delta(1))

//when we're putting pixels, we
//then must check if is_steep, in 
//which case we must put the pixels
//in reverse order essentially undo
//the swap above.
void rt_put_line(rt_ctx_t *rt_ctx,
                     v3 p0, v3 p1, u32 c)
{
    v3 p0s, p1s;
    if (!trans_world_to_screen(p0, &p0s)) return;
    if (!trans_world_to_screen(p1, &p1s)) return;
    DEBUG("p0s = %f %f | p1f = %f %f",
          p0s.x, p0s.y, p1s.x, p1s.y);

    i32 x0 = (i32)p0s.x;
    i32 y0 = (i32)p0s.y;
    i32 x1 = (i32)p1s.x;
    i32 y1 = (i32)p1s.y;

    bool is_steep = abs(y1 - y0) > abs(x1 - x0); 

    if (is_steep) {
        SWAP(x0, y0);
        SWAP(x1, y1);
    }

    if (x0 > x1) {
        SWAP(x0, x1);
        SWAP(y0, y1);
    }

    i32 dx = x1 - x0;
    i32 dy = abs(y1 - y0);
    i32 yi = (y1 - y0 < 0) ? -1 : 1;
    
    i32 diff = (2 * dy) - dx;
    i32 y = y0;

    for (i32 x = x0; x < x1; x++) {
        DEBUG("DIFF: %d", diff);

        if (is_steep) rt_put_pixel(rt_ctx, y, x, 0, c);
        else          rt_put_pixel(rt_ctx, x, y, 0, c);

        if (diff >= 0) {
            y += yi;
            diff += (2 * (dy - dx));
        } else {
            diff += (2 * dy);
        }
    }
}

void rt_put_tri(rt_ctx_t *rt_ctx,
                v3 p1, v3 p2, v3 p3, u32 c)
{
    //temp
    (void)c;
    v3 p1s, p2s, p3s;
    if (!trans_world_to_screen(p1, &p1s) || 
        !trans_world_to_screen(p2, &p2s) ||
        !trans_world_to_screen(p3, &p3s)) {
        TRACE("returning early");
        return;
    } 

    aabb_t tri_aabb = aabb_get(p1s, p2s, p3s);
    TRACE("AABB: min %f %f -- max %f %f",
          tri_aabb.min.x, tri_aabb.min.y,
          tri_aabb.max.x, tri_aabb.max.y);
    ;
    for (u32 x = tri_aabb.min.x; x < tri_aabb.max.x; x++) {
        for (u32 y = tri_aabb.min.y; y < tri_aabb.max.y; y++) {
            v3 bary = barycentric(p1s, p2s, p3s, (v3){x, y, 0});
            TRACE("BARY: %f %f %f", bary.x, bary.y, bary.z);
            u32 inter_c = RGBA_TO_HEX((u8)(255*bary.x), 
                                      (u8)(255*bary.y),
                                      (u8)(255*bary.z), 1);
            if (!v3_contains_neg(bary)) {
                rt_put_pixel(rt_ctx, x, y, 0, inter_c);
            }
        }
    }
}

